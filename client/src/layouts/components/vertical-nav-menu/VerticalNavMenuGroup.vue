...
<template>
  <div
    ...
    @mouseout  = "mouseout">
      ...
      <!-- Group Label -->
      <div @click="clickGroup" class="group-header w-full" v-if="cansee">
      ...
      <ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
        <li v-for="(groupItem, index) in group.submenu" :key="index">
          ...
          <v-nav-menu-item
            ...
            :to="groupItem.type !== 'external' ? groupItem.url : null"
            :href="groupItem.type === 'external' ? groupItem.url : null"
...
export default {
  ...
  created() {
    this.hasItem(this.group)    
  },
  methods: {
    async hasItem(item){
      const userD = JSON.parse(this.userPemData['user_role']['permission'])

      const matchingKey = Object.keys(userD).find(key =>  
        Object.keys(item['submenu']).find(itemKey => 
          key+'-list' === item['submenu'][itemKey]['slug'] && userD[key]['list']
        )
      )

      // if (key+'-list' === item['submenu'][itemKey]['slug'] && userD[key]['list']) {
      //   console.log("userD[key] ", userD[key]['list'])
      //   console.log("key+'-list' ", key+'-list')
      //   console.log("item['submenu'][itemKey]['slug'] ", item['submenu'][itemKey]['slug'])
      //   console.log('ismatched ', key+'-list' === item['submenu'][itemKey]['slug'])
      //   return key
      // }

      this.cansee = matchingKey !== undefined && userD[matchingKey]['list'] !== undefined ? userD[matchingKey]['list'] : false
    },
    clickGroup () {
      EventBus.$emit('getCurrentMenuItem', this.group)
      if (!this.openHover) {
