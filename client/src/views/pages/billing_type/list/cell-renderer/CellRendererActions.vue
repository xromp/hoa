...
export default {
  ...
  methods: {
    editRecord () {
      this.$router.push("/billing-types/edit/"+ this.params.data.id)
    },
    confirmDeleteRecord () {
      this.$vs.dialog({
        type: 'confirm',
    ...
    async deleteRecord () {
      try {
        const result = await axios.post(`/bill-type/delete/${this.params.data.id}`, null, {
          headers: {
        ...
        EventBus.$emit('bill-type-list', true)
      } catch (err) {
        this.$vs.notify({
          time:10000,
          color: 'danger',
