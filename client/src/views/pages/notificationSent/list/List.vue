...
export default {
  ...
  data () {
    ...
    return {
      ...
      columnDefs: [
        ...
        // },
         {
          //notification
          headerName: 'Title',
          field: 'notification.notification_message.title',
          width: 400,
          filter: true
        },
        {
          //notification
          headerName: 'Message',
          field: 'notification.notification_message.message',
          width: 400,
          filter: true
        },
        {
        ...
        //   //notification
        //   headerName: 'Created',
        //   field: 'notification.notification_message.createdAt',
        //   width: 150,
        //   filter: true,
  ...
  methods: {
    ...
    async loadData() {
      this.$vs.loading()
      const result = await axios.get('/notification-sent/list', {
        headers: { 
  ...
  mounted () {
    EventBus.$on('update-parent-id',async (result) => {
      if (this.$route.path !== '/notification-sents/list') return      
      console.log('/notification-sents/list')
      await this.loadData()
    })
  ...
  async created () {
    try {
      //crud
      this.showL = await mainHelper.cansee('notification-sent-list', 'list')
      this.showC = await mainHelper.cansee('notification-sent-create', 'create')
      this.showR = await mainHelper.cansee('notification-sent-read', 'read')
      this.showU = await mainHelper.cansee('notification-sent-update', 'update')
      this.showD = await mainHelper.cansee('notification-sent-delete', 'delete')
