    ...
    <div class="vx-row">
      <div class="vx-col md:w-1/2 w-full">
        ...
        <div class="mt-4">
          ...
          <v-select class="loading-container-po" :options="optionParentOrg" :reduce="name => name.id" label="name" v-validate="'required'" v-model="data_local.pmc_id" name="pmc_id" disabled/>
          <span class="text-danger text-sm"  v-show="errors.has('pmc_id')">{{ errors.first('pmc_id') }}</span>
        </div>
      </div>

      <div class="vx-col w-full" :class="[data_local.property_type === 'multiple' ? 'md:w-1/4': 'md:w-1/2']">
        <div class="mt-4">
          <label class="vs-input--label">Property Type*</label>
          <v-select :options="optionPropertyType" :reduce="name => name.val" label="name" v-validate="'required'" v-model="data_local.property_type" name="property_type"/>
          <span class="text-danger text-sm"  v-show="errors.has('property_type')">{{ errors.first('property_type') }}</span>
        </div>


        <!-- <div class="mt-4">
          <vs-input class="w-full mt-4" label="Radius*" v-model="data_local.radius" type="text" v-validate="'required'" name="radius" />
          <span class="text-danger text-sm"  v-show="errors.has('radius')">{{ errors.first('radius') }}</span>
        </div> -->
      </div>
      <building-selection
        ...
        <div>
          <div class="vx-col w-full">
            ...
            <div class="mt-4 country-option">
              ...
              <label class="vs-input--label">Country*</label>
              <v-select :options="optionCountry" :reduce="name => name.id" label="name" v-validate="'required'" v-model="data_local.BillingCountry" name="BillingCountry" :disabled="is_copy_to_billing" autocomplete="chrome-off"/>
              <span class="text-danger text-sm"  v-show="errors.has('BillingCountry')">{{ errors.first('BillingCountry') }}</span>
            </div>
...
export default {
  ...
  methods: {
    ...
    async save_changes (loc) {   
      try {
        ...
        this.data_local.ref = Buffer.from(this.data_local.name).toString('hex')
        this.data_local.created_by_manager_id = decoded.id
        this.data_local.createdAt = new Date()
        const editId = -1
        let pid = []

        const result = await axios.post(`/property/save/${editId}`, {
          data: this.data_local,
          token: this.token
        })

        this.resDataLocal = result['data']['result']
        pid.push(this.resDataLocal.id)
        ...
        await axios.post(`/docs/table/save`, {
          ...
          path: `docs/property/avatar/${this.resDataLocal.id}`,
          folderId: 26
        })
        ...
        await axios.post(`/user-org/add`,
          { 
            data: {
              ...
              pmc_id: this.data_local.pmc_id, 
              property_id: this.resDataLocal.id, 
              unit_id: 0
            }
          },
    ...
    reset_data () {
      ...
      this.data_local['radius'] = ''
      this.data_local['code'] = ''
      this.data_local['is_deleted'] = ''
      this.data_local['created_by_manager_id'] = ''
      this.data_local['manager_id'] = ''
      this.data_local['property_type'] = ''
      this.data_local['permissions1_json'] = ''
      this.data_local['permissions2_json'] = ''
      this.data_local['domain'] = ''
      this.data_local['pmc_id'] = ''
      this.data_local['photo_filename'] = ''
      this.data_local['ref'] = ''
      this.data_local['settings_json'] = ''
      this.data_local['email_signature'] = ''
      this.data_local['caliber_client_id'] = ''
      this.data_local['BillingAddress1'] = ''
      this.data_local['BillingAddress2'] = ''
      this.data_local['BillingAddress3'] = ''
      this.data_local['BillingCity'] = ''
      this.data_local['BillingCountry'] = ''
      this.data_local['BillingFax'] = ''
      this.data_local['BillingPhoneArea'] = ''
      this.data_local['BillingPhone'] = ''
      this.data_local['BillingPhoneType'] = ''
      this.data_local['BillingState'] = ''
      this.data_local['BillingZip'] = ''
      ...
      this.$vs.notify({
        color: 'orange',
        title: 'Edit',
        text: 'The selected Data was reset'
      })
    },
    ...
    async loadParentOrg () {      
      ...
      setTimeout(() => {
        this.$vs.loading.close("div.loading-container-po > .con-vs-loading")
        this.loading = false
      }, 1000)
    },
    async changeProperty() {      
      ...
      localStorage.setItem('selectedPropertyRef', this.resDataLocal.ref)

      console.log('this.resDataLocal.ref ', this.resDataLocal.ref)
      await mainHelper.refreshToken();

      this.$vs.loading()
      this.resDataLocal.actionType = 'add'
      EventBus.$emit('side-menu-property', this.resDataLocal)
    },
  },
  async created () {
    try {
  ...
  mounted () {
    ...
    EventBus.$on('update-parent-id', async (result) => {
      if (this.$route.path !== `/properties/new`) return
      console.log(`/properties/new update-parent-id`)
      this.loadParentOrg()
      this.data_local.pmc_id = parseInt(localStorage.selectedParentOrg)
    ...
    EventBus.$on('user-org-list',async (result) => {
      if (this.$route.path !== `/properties/new`) return
      console.log(`/properties/new user-org-list`)
      this.loadParentOrg()
      this.data_local.pmc_id = parseInt(localStorage.selectedParentOrg)
      setTimeout(() => { this.$vs.loading.close() },1000)
    }) 
