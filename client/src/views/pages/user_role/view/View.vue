<template>
  <div id="page-data-view">
    ...
    <div v-if="data_not_found===1">
      <vx-card title="User Role Details" class="mb-base">
        <div class="vx-row">
          ...
          <div class="vx-col w-full flex" id="account-manage-buttons">
            <vs-button icon-pack="feather" icon="icon-edit" class="mr-4" :to="{name: 'app-role-edit', params: { editId: $route.params.viewId }}" 
      ...
      <vx-card title="User Sub Role" class="mb-base">
        <list-tab class="mt-4" />
      </vx-card>  
    </div>    
  </div>
...
export default {
  ...
  mounted () {   
    EventBus.$on('update-parent-id',async (result) => {
      ...
      if (this.$route.path !== `/user-types/view/${this.$route.params.viewId}`) return
      console.log('/user-types/view/')
      await this.loadData()
    })   
