...
export default {
      ...
      columnDefs: [
        {
          ...
          cellClass: params => params.data.parent_id !== 0 && 'sub-role'
        },
        {
          //role
  ...
  methods: {
    ...
    newRecord () {
      this.$router.push("/user-types/new")
    },
    async loadData() {
      this.$vs.loading()
  ...
  mounted () {
    EventBus.$on('update-parent-id',async (result) => {
      setTimeout(async () => { this.$vs.loading.close() },1000)
      if (this.$route.path !== '/user-types/list') return
      console.log('/user-types/list')
      await this.loadData()
    })
    ...
    this.rowClassRules = {
      'parent-role': 'data.parent_id === 0',
    }
    this.gridApi = this.gridOptions.api
    this.gridApi.setDomLayout('autoHeight');
    this.gridApi.sizeColumnsToFit();
    document.querySelector('#userRoleGrid').style.height = '100%';
