...
const roleVal = localStorage.selectedRoleVal
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          width: 150,
          filter: true,
          hide: roleVal !== 'admin' && roleVal !== 'staff'
        },
        {
          //order
          headerName: 'Due Date',
  ...
  methods: {    
    ...
    newRecord () {
      this.$router.push("/orders/new?t=1")
    },
    async loadData() {
      this.$vs.loading()
  ...
  mounted () {
    EventBus.$on('update-parent-id',async (result) => {
      if (this.$route.path !== `/orders/list`) return
  ...
  async created () {
    try {
      ...
      this.showPage = await mainHelper.isPageAvailable('order')
      if (this.showPage) { this.$router.push({name:'page-coming-soon'}) }
