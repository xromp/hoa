<template>
  <div id="page-data-list">
    ...
    <div class="vx-card p-6">
      ...
      <vs-popup classContent="popup-example" title="Assign New User" :active.sync="popupActive2">
        <div class="vx-col w-full h-full pop-owner-resident">
          <div>  
            <div>
              <label class="vs-input--label">User Email</label>
              <v-select multiple :options="optionUser" :reduce="email => email.id" label="email" v-validate="'required'" v-model="data_local.user_id" name="user_id" class="inputx"/>
            ...
            <div class="mt-4">
              ...
              <v-select :options="optionParentOrg" :reduce="name => name.id" label="name" v-validate="'required'" v-model="localParentOrg" name="localParentOrg" class="inputx" disabled/>
            </div>
            <div class="mt-4">
              <label class="vs-input--label">Property</label>
              <v-select :options="optionProperty" :reduce="name => name.id" label="name" v-validate="'required'" v-model="data_local.property_id" name="property_id" class="inputx" disabled/>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      localParentOrg: parseInt(localStorage.selectedParentOrg),
  ...
  methods: {
    ...
    async loadParentOrg() {
      const result = await axios.get(`/portfolio/list/`,
        { headers: { 'Content-Type': 'application/json', 'Authorization': this.token }}
      )
    ...
    async loadProperty () {
      const result = await axios.get('/property/list', {
        ...
        params: { 
          'pmc_id': this.$route.params.viewId,
      ...
      this.optionProperty.filter((res) => {
        this.data_local.property_id = localStorage.selectedPropertyRef == res['ref'] ? res['id'] : this.data_local.property_id
      })
    },
    async loadUser () {
