    ...
    <div class="vx-row">
      <div class="vx-col md:w-1/2 w-full">
        ...
        <div class="mt-4">
          ...
          <v-select :options="optionUserRole" :reduce="name => name.id" label="name" v-model="role_id" name="salutation_id" disabled/>
        </div>

        <vs-input class="w-full mt-4" label="Notes" v-model="data_local.notes" name="notes" />
      </div>

      <div class="vx-col md:w-1/2 w-full" v-if="roleVal==='vendor'">
         <div class="vx-col w-full mt-4">
          <label class="vs-input--label">Radius*</label>
          <div id="map"></div>
          <vs-input v-model="data_local.vendor_user.vendor.service_radius" class="w-full mt-4" name="service_radius" v-validate="'required'" type="hidden"/>
          <span class="text-danger vx-row ml-0">{{ errors.first('step-1.service_radius') }}</span>
          <!-- <vs-button id="clear-map" color="success" class="mt-2">Reset</vs-button> -->
...
export default {
  ...
  methods: {
    ...
    async getAddressData({ country, locality, postal_code, administrative_area_level_1, latitude, longitude }, { formatted_address }, type) {
      if (this.roleVal ==='vendor') {   
        $('#center-lat').val(latitude);
        $('#center-lng').val(longitude);

        googleMapsDrawing.initialize(
          this.service_radius.coord, 
          parseFloat(latitude), 
          parseFloat(longitude), 
          parseFloat(0)
        )  
        this.data_local.vendor_user.vendor.service_radius = {
          center_lat: latitude,
    ...
    async save_changes () {
      try {
        ...
        if (this.roleVal ==='vendor') {
          await axios.post(`/vendor/save/${this.data_local.vendor_user.vendor.id}`, {
            data: this.data_local.vendor_user.vendor,
            token: this.token
          })
        }
        ...
        this.$vs.loading.close()
        // this.$router.go(-1)
        window.location.href = `/user-profile/edit/${editId}`;
      } catch(err) {
        this.$vs.notify({
          color: 'danger',
          title: 'Saving Data',
    ...
    async loadServiceRadius() {
      if (this.roleVal ==='vendor') {
        this.service_radius = JSON.parse(this.data_local.vendor_user.vendor.service_radius)

        $('#center-lat').val(this.service_radius.center_lat);
        $('#center-lng').val(this.service_radius.center_lng);
        $('#center-radius').val(this.service_radius.center_radius);

        googleMapsDrawing.initialize(
          this.service_radius.coord, 
          parseFloat(this.service_radius.center_lat), 
          parseFloat(this.service_radius.center_lng), 
          parseFloat(this.service_radius.center_radius)
        )   
      }
    }
  },
