  ...
  <div id="page-data-list">
    <vx-card ref="filterCard" title="Filters" class="data-list-filters mb-8" actionButtons @refresh="resetColFilters" @remove="resetColFilters">
      <div class="vx-row">
        <div class="vx-col md:w-1/4 sm:w-1/4 w-full">
          ...
          <v-select :options="optionStatus" label="name" v-validate="'required'" v-model="categoryFilter"/>
        </div>
      </div>
    </vx-card>
    ...
    <div class="vx-card p-6">
      <div class="flex flex-wrap items-center">
        ...
        <!-- IMPORT USER -->
        <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-dashed border-primary mt-4" @click="importUser">
...
export default {
  ...
  watch: {
    ...
    categoryFilter(obj) {
      this.setColumnFilter('status_val', obj.name)
    },
  },
  computed: {    
    paginationPageSize () {
  ...
  methods: {
    ...
    async loadData() {
      ...
      for (const [key, value] of Object.entries(result['data'])) {
        value.status_val = value.is_deleted === 0 ? "Active" : "Archived"
      }

      this.listData = result['data']
      console.log('this.listData ', this.listData)
        // .filter(({ user_org_role }) => !user_org_role.is_clone)
        // .map(user => ({ ...user, unit_number: user.unit_users.map(({ unit }) => unit.number).join() }));
      setTimeout(async () => { 
        this.$vs.loading.close() 
        this.categoryFilter = { id: 2, name: "All"}
      },1000)
    },
    async loadUnitUser(user_id) {
      this.$vs.loading()
      const {data} = await axios.get(`/unit/user/list/${user_id}`, {
        headers: { 
          'Authorization': this.token
        },
        params: {
  ...
  mounted () {
    EventBus.$on('update-parent-id',async (result) => {
      if (this.$route.path !== `/users/list`) return
    ...
    EventBus.$on('refetch-user',async (result) => await this.loadData())
