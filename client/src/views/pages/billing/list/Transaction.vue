...
import CellRendererPayee from './cell-renderer/CellRendererPayee.vue'
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          width: 200,
          filter: true,
          valueFormatter: ({ value }) => this.numberFormat(value),
        },
      ],
  ...
  methods: {
    ...
    newRecord () {
      this.$router.push('/billings/new?type=ar')
    },
    async loadTransaction() {
      try {
        ...
        const transaction = data
          .filter(({ trans_account_id }) =>  trans_account_id === 1)
          .map(trans => ({ ...trans, description: `Current ${trans.description} Balance: `}));
        this.listData = transaction[0].transaction_line_v2s;
        this.pinnedTopRowData = transaction;
