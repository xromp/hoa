  ...
  <div id="page-data-list">
    ...
    <div class="vx-card p-6">
      ...
      <div class="flex flex-wrap items-center">
        ...
        <!-- ADD NEW -->
        <!-- <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-primary border border-solid border-primary mt-4" @click="newRecord" v-if="showC"> --> <!--crud-->
...
import CellRendererEditModal from './cell-renderer/CellRendererEditModal.vue'
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          //amenity
          headerName: 'Name',
          field: 'amenity_reservation.user.full_name',
          width: 300,
          filter: true
        },
        {
          //amenity
          headerName: 'Unit',
          field: 'amenity_reservation.unit.number',
          width: 100,
          filter: true
        },
        {
          ...
          width: 200,
          filter: true,
          cellRendererFramework: 'CellRendererEditModal'
        },
        {
          //amenity
  ...
  methods: {
    ...
    async loadAmenityTime () {
      try {
        const res = await axios.get('/api/amenity-time/list', {
          headers: { 'Authorization': this.token, 'property_ref': localStorage.selectedPropertyRef }
        })

        // this.optionMinHours = res['data']
        this.optionMaxHours = res['data']
        // this.optionBufferTime = res['data']
      } catch (err) {
    ...
    async loadList () {
      const d = new Promise(async (resolve, reject)  => {
        try {
          await axios.get('/amenity-reservation/list', {
            headers: { 'Authorization': this.token, 'property_ref': localStorage.selectedPropertyRef }
          })
            .then(res => {
        ...
        d.then((res) => {
          res.filter(ar => {
            ar['res_time'] = (`${ar['from_time']  } to ${  ar['to_time']}`)
            ar['from_date'] = moment(new Date(ar['from_date'])).format('MM/DD/YYYY')
          })
  ...
  async created () {
    this.loadAmenityTime()
    this.loadList()
    //crud
    this.showL = await mainHelper.cansee('reservation-list', 'list')
    this.showC = await mainHelper.cansee('reservation-create', 'create')
    this.showR = await mainHelper.cansee('reservation-read', 'read')
