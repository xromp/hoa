  ...
  <div id="page-data-list">
    ...
    <vx-card ref="filterCard" title="Filters" class="data-list-filters mb-8" actionButtons @refresh="resetColFilters" @remove="resetColFilters">
      <div class="vx-row">
        ...
        <div class="vx-col md:w-1/4 sm:w-1/4 w-full">
          ...
          <v-select :options="optionVendorCategoryMaster" label="name" v-validate="'required'" v-model="categoryFilter"/>
        </div>
      </div>
    </vx-card>
    ...
    <div class="vx-card p-6">
      ...
      <div class="flex flex-wrap items-center">
        ...
        <vs-input class="sm:mr-4 mr-0 sm:w-auto w-full sm:order-normal order-3 sm:mt-0 mt-4" v-model="emailInvite" placeholder="Enter email here" />
        <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary mt-4" @click="inviteVendor">
            <feather-icon icon="PlusIcon" svgClasses="h-4 w-4" />
            <span class="ml-2 text-base text-primary">Invite</span>
        </div>
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
          //vendor
          headerName: 'Category',
          field: 'vendor_category_master.name',
          width: 200,
          filter: true
        },
        {
  ...
  watch: {
    ...
    categoryFilter(obj) {
      this.setColumnFilter('vendor_category_master.name', obj.name)
    },
  },
  computed: {
  ...
  methods: {
    ...
    async inviteVendor() {
      try {        
        await axios.post(`/vendor/invite`, {
        ...
        this.$vs.notify({
          color: 'success',
          title: 'Data Sent',
          text: 'Invitation was successfully sent'
        })
    ...
    loadVendorCategoryMaster () {
      ...
      d.then((res) => {
        ...
        this.optionVendorCategoryMaster.unshift({'name': 'All'})
      })
    },
  },
  mounted () {
  ...
  created () {
    ...
    var d = new Promise(async(resolve, reject)  => {
      try {    
        await axios.get('/vendor/property/list', {
            headers: { 'Authorization': this.token }
          })
          .then(res => { 
