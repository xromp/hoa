...
export default {
  ...
  methods: {
    async loadUserPendingRequest() {
      const { data } = await axios.get('/user-pending-request/get-user-requests', {
        headers: { 'Authorization': localStorage.usertoken }
      });
      this.usersData = data.map(({ id, status, requestDataObj }) => {
        const { email, first_name, last_name, unit, role } = JSON.parse(requestDataObj);
        return {
          ...JSON.parse(requestDataObj),
          unit_id: optionalChaning(() => unit.id),
          role_id: optionalChaning(() => role.id),
          id,
          unitName: optionalChaning(() => unit.number),
          roleName: optionalChaning(() => role.name),
          email,
