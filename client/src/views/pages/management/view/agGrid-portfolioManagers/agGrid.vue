      ...
      <vs-popup classContent="popup-example" title="Add New User" :active.sync="popupActive2">
        <div class="vx-col w-full h-full">
          <div>
            ...
            <v-select multiple :options="optionUser" :reduce="email => email.id" label="email" v-validate="'required'" v-model="value1" name="user_id" class="inputx"/>
          </div>

          <div class="mt-6">            
            <vs-button class="mr-2" @click="newRecord" color="primary" type="filled">Add</vs-button>
...
export default {
  ...
  methods: {
    ...
    newRecord () {
      ...
      this.$router.push(`/users/new?r=pmcm&rid=16&pmcid=${this.$route.params.viewId}`)
    },
    loadUser () {
  ...
  created () {
    ...
    var d = new Promise(async(resolve, reject)  => {
      try {    
          await axios.get('/portfolio-manager/list', {
              headers: { 
                'Authorization': this.token,
                'pmc_id': viewId
              }
